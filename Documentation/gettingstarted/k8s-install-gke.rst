...
You might also want to pass the `--set global.restartPods` switch to the `helm`
command above. This will restart existing pods when initializing the node to
force all pods to be managed by Cilium.
